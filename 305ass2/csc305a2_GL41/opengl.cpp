...
static std::map<void*, const char*> GLUnimplementedProcToName;

template<uint32_t H, class F> struct UnimplementedGL;
template<uint32_t H, class R, class... Args>
struct UnimplementedGL<H, R(Args...)>
{
    static R GLAPIENTRY call(Args...)
    {
        fprintf(stderr, "The function %s is not implemented in your computer's GL version.\n", GLUnimplementedProcToName.at((void*)&call));
        fprintf(stderr, "BTW, you can't just check if the function is null. glxGetProcAddress never returns NULL function ptrs.\n");
        fprintf(stderr, "You should do a proper version/extension feature check.\n");
        BREAKPOINT;
        return R();
    }
};
template<uint32_t H, class... Args>
struct UnimplementedGL<H, void(Args...)>
{
    static void GLAPIENTRY call(Args...)
    {
        fprintf(stderr, "This function %s is not implemented in your computer's GL version.\n", GLUnimplementedProcToName.at((void*)&call));
        fprintf(stderr, "BTW, you can't just check if the function is null. glxGetProcAddress never returns NULL function ptrs.\n");
        fprintf(stderr, "You should do a proper version/extension feature check.\n");
        BREAKPOINT;
    }
...
static std::map<void*, void*> GLWrappedGetErrorProcToProc;
static std::map<void*, const char*> GLWrappedGetErrorProcToName;

template<uint32_t H, class F> struct WrapGetErrorGL;
template<uint32_t H, class R, class... Args>
struct WrapGetErrorGL<H, R(Args...)>
{
    static R GLAPIENTRY call(Args... args)
    {
        R r = reinterpret_cast<R(GLAPIENTRY*)(Args...)>(GLWrappedGetErrorProcToProc.at((void*)&call))(args...);
        CheckErrorGL(GLWrappedGetErrorProcToName.at((void*)&WrapGetErrorGL<H,R(Args...)>::call));
        return r;
    }
};
template<uint32_t H, class... Args>
struct WrapGetErrorGL<H, void(Args...)>
{
    static void GLAPIENTRY call(Args... args)
    {
        reinterpret_cast<void(GLAPIENTRY*)(Args...)>(GLWrappedGetErrorProcToProc.at((void*)&call))(args...);
        CheckErrorGL(GLWrappedGetErrorProcToName.at((void*)&WrapGetErrorGL<H,void(Args...)>::call));
    }
};

#ifdef _DEBUG
static bool GLDebugOutputSupported = false;
#endif

template<uint32_t H, class F>
void GetProcGL(F*& proc, const char* name)
{
    proc = reinterpret_cast<F*>(SDL_GL_GetProcAddress(name));
    if (!proc)
    {
        GLUnimplementedProcToName[(void*)&UnimplementedGL<H,F>::call] = name;
        proc = UnimplementedGL<H,F>::call;
    }
#ifdef _DEBUG
    else
    {
        if (!GLDebugOutputSupported && strcmp(name, "glGetError") != 0)
        {
            GLWrappedGetErrorProcToProc.emplace((void*)&WrapGetErrorGL<H, F>::call, (void*)proc);
            auto wrappedName = GLWrappedGetErrorProcToName.emplace((void*)&WrapGetErrorGL<H, F>::call, name);
            if (!wrappedName.second)
            {
                fprintf(stderr, "Hash collision in WrapGetErrorGL. %s collided with %s\n", name, wrappedName.first->second);
            }
            proc = WrapGetErrorGL<H,F>::call;
        }
    }
#endif
}
...
// Cheers to https://github.com/elanthis/constexpr-hash-demo/blob/master/test.cpp
// for the constexpr hashing code
...
constexpr uint32_t hash_one(char c, const char* remain, uint32_t value)
{
    return c == 0 ? value : hash_one(remain[0], remain + 1, uint32_t((value ^ c) * uint64_t(uint32_t(prime))));
}

// compile-time hash
constexpr uint64_t hash(const char* str)
{
    return hash_one(str[0], str + 1, uint32_t(basis));
}

#define GET_PROC_GL(n) GetProcGL<hash(#n)>(n, #n)

void OpenGL_Init()
{
#ifdef _DEBUG
    // enable glDebugMessageCallback if available
    {
        auto pfnglGetIntegerv = (PFNGLGETINTEGERVPROC)SDL_GL_GetProcAddress("glGetIntegerv");

        GLint major, minor;
        pfnglGetIntegerv(GL_MAJOR_VERSION, &major);
        pfnglGetIntegerv(GL_MINOR_VERSION, &minor);
        if ((major > 4 || (major == 4 && minor >= 3)))
        {
            GLDebugOutputSupported = true;
            auto pfnglDebugMessageCallback = (PFNGLDEBUGMESSAGECALLBACKPROC)SDL_GL_GetProcAddress("glDebugMessageCallback");
            pfnglDebugMessageCallback(DebugCallbackGL, NULL);
            auto pfnglEnable = (PFNGLENABLEPROC)SDL_GL_GetProcAddress("glEnable");
            pfnglEnable(GL_DEBUG_OUTPUT_SYNCHRONOUS_ARB);
        }
        else if (SDL_GL_ExtensionSupported("GL_ARB_debug_output"))
        {
            GLDebugOutputSupported = true;
            auto pfnglDebugMessageCallbackARB = (PFNGLDEBUGMESSAGECALLBACKARBPROC)SDL_GL_GetProcAddress("glDebugMessageCallbackARB");
            pfnglDebugMessageCallbackARB(DebugCallbackGL, NULL);
            auto pfnglEnable = (PFNGLENABLEPROC)SDL_GL_GetProcAddress("glEnable");
            pfnglEnable(GL_DEBUG_OUTPUT_SYNCHRONOUS);
        }
    }
#endif
